...
1. **Classify stakes.** trivial → light check, ship. non-trivial → full gate below.
   irreversible/MAJOR → full gate + human approval before acting.
...
2. **Decide fan-out (bias-to-NO on agents).** Use a single strong agent by default.
   ...
   Subagents explore in clean contexts and return ~1–2k-token condensed summaries.
...
| "More agents = better" | Mean multi-agent gain across tasks is −3.5%. Scale to the task; default single. |
